...
MAIN_PATH   = r"C:\Users\aksha\OneDrive\Documents\Vtitan\Eletro seth\motorola motorola edge 50 pro (1b991842ca3f82c3)\motorola motorola edge 50 pro (1b991842ca3f82c3)\1 Testing\Heart_file.wav"       # stethoscope mic
...
def process(main_path: str, anc_path: str, output_dir: str = "."):
    ...
    print(f"Alpha (over-sub): {OVER_SUB_ALPHA}")
    print(f"Beta  (floor)   : {FLOOR_BETA}")

    # ── Step 1: Spectral subtraction — remove ambient noise ──────────────────
    print("\n[1/3] Running spectral subtraction...")
    clean = spectral_subtraction(main, anc, fs,
                                  chunk_ms=CHUNK_MS,
                                  alpha=OVER_SUB_ALPHA,
                                  beta=FLOOR_BETA)

    # ── Step 2: Heart mode — 50–200 Hz, stateful bandpass ───────────────────
    print("[2/3] Filtering heart mode  (50–200 Hz)...")
    b_h, a_h = butter_bandpass(HEART_LOW, HEART_HIGH, fs, order=4)
    heart_out = apply_filter_stateful(b_h, a_h, clean, chunk_size)

    # ── Step 3: Lung mode — 100–1500 Hz, stateful bandpass ──────────────────
    print("[3/3] Filtering lung mode (100–1500 Hz)...")
    ...
    def save(filename, data):
        ...
        print(f"   Saved: {path}  (RMS={rms:.1f})")

    print("\nSaving outputs...")
    save("heart_mode_replicated.wav", heart_out)
    save("lung_mode_replicated.wav",  lung_out)
    save("anc_clean_residual.wav",    clean)      # The noise-reduced main (before mode split)
