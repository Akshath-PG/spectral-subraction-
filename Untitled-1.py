...
Method: Frequency Domain Subtraction in 30ms chunks
  For each chunk:
    1. Compute complex FFT of main chunk and anc chunk
    2. Subtract anc complex spectrum from main complex spectrum (magnitude AND phase)
    3. Apply spectral floor on the residual magnitude to suppress musical noise
    4. Reconstruct signal via IFFT
    5. Bandpass filter to the mode-specific range

  Unlike spectral subtraction (which only removes magnitude and keeps main's
  original phase), this subtracts the full complex spectrum — the residual
  phase emerges naturally from the subtraction, giving more aggressive and
  accurate ambient cancellation.
...
ANC_PATH    = r"C:\Users\aksha\OneDrive\Documents\Vtitan\Eletro seth\motorola motorola edge 50 pro (1b991842ca3f82c3)\motorola motorola edge 50 pro (1b991842ca3f82c3)\1 Testing\Anc_mic.wav"
OUTPUT_DIR  = r"C:\Users\aksha\OneDrive\Documents\Vtitan\Eletro seth\Spectral subtaction\Outputs\Result"
...
# ─────────────────────────────────────────────────────────────────────────────
# CORE: FREQUENCY DOMAIN SUBTRACTION IN 30ms CHUNKS
# ─────────────────────────────────────────────────────────────────────────────

def frequency_domain_subtraction(main: np.ndarray, anc: np.ndarray,
                                  fs: int, chunk_ms: int = 30,
                                  alpha: float = 1.2, beta: float = 0.02) -> np.ndarray:
    """
    Remove ambient noise by subtracting the ANC complex spectrum directly
    from the main signal's complex spectrum (magnitude AND phase).

    Unlike spectral subtraction (which only subtracts magnitude and keeps
    main's original phase), this modifies the complex FFT directly — the
    residual phase emerges naturally from the subtraction, giving more
    accurate ambient cancellation when the reference mic closely tracks
    ...
    alpha     : over-subtraction factor (1.0–2.0). Higher = more aggressive.
                Reduce to ~1.0 if heart/lung output sounds hollow.
    ...
    for i in range(n_chunks):
        ...
        # Complex FFT of both chunks
        main_fft = np.fft.rfft(main_chunk)
        anc_fft  = np.fft.rfft(anc_chunk)

        # ── FREQUENCY DOMAIN SUBTRACTION ───────────────────────────────────
        # Subtract full complex spectra (magnitude AND phase of ambient removed).
        # The residual phase emerges naturally — no need to borrow main's phase.
        residual_fft = main_fft - alpha * anc_fft

        # Extract residual magnitude and phase
        residual_mag   = np.abs(residual_fft)
        residual_phase = np.angle(residual_fft)
        main_mag       = np.abs(main_fft)

        # Apply spectral floor on residual magnitude to suppress musical noise.
        # This is the same floor as before, now applied post-subtraction.
        floored_mag = np.maximum(residual_mag, beta * main_mag)

        # Reconstruct using floored magnitude + naturally emerged residual phase
        clean_fft = floored_mag * np.exp(1j * residual_phase)
        # ───────────────────────────────────────────────────────────────────
...
def process(main_path: str, anc_path: str, output_dir: str = "."):
    ...
    Full pipeline:
      1. Load main + anc
      2. Frequency domain subtraction (ambient removal) in 30ms chunks
      3. Bandpass filter into Heart mode (50–200 Hz)
      4. Bandpass filter into Lung mode  (100–1500 Hz)
      5. Save outputs + spectrograms
    """
    print("=" * 55)
    print("GFANC Stethoscope — Python Replication")
    print("Method: Frequency Domain (Complex) Subtraction")
    print("=" * 55)
    ...
    # ── Step 1: Frequency domain subtraction ──────────────────────────────
    print("\n[STEP 1/3] STARTED  - Frequency Domain Subtraction")
    print("  Subtracting complex spectrum of anc from main...")
    print("  Processing in 30ms chunks with 50% overlap-add...")
    clean = frequency_domain_subtraction(
        main, anc, fs,
        chunk_ms=CHUNK_MS,
        alpha=OVER_SUB_ALPHA,
        beta=FLOOR_BETA
    )
    print("[STEP 1/3] COMPLETE - Frequency Domain Subtraction done")
    ...
    for ax, (title, sig) in zip(axes, signals):
        ...
        Pxx, freqs, bins, im = ax.specgram(
            sig, NFFT=1024, Fs=fs, noverlap=512, cmap=audacity_cmap
        )
